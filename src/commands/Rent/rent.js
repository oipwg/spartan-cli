...
export default function(vorpal, options){
	...
	.action(async function (args, cb) {
		...
		async function interactiveObjectInspection(object) {
			...
			let modifiedKeys = []
			for (let key of keys) {
				modifiedKeys.push(key += `: ${vorpal.chalk.yellow(object[key])}`)
			}
			...
			if (inspectionPrompt.option === exit)
				return true

			let param = inspectionPrompt.option
			let splited = param.split(':')
			param = splited[0]
			if (Array.isArray(object[param]) || typeof object[param] === 'object') {
		...
		async function onRentalFnFinish(rental_info) {
			switch(rental_info.status) {
				...
				case WARNING:
					...
					break
				case ERROR:
					self.log(vorpal.chalk.red(`There was an error attempting to rent.`))
